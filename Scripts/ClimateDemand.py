...
cols = list(set(ClimateTMAX.columns).intersection(ClimateTMIN.columns).intersection(ClimatePRCP.columns))

DiversionTotal = pd.DataFrame(index=ClimateTMAX.index)

ModelResults = []

ObservedDiversions = pd.DataFrame(index=ClimateTMAX.index, columns=Reaches['RiverWare Reach'].unique()).fillna(0)
...
for Reach in Reaches['RiverWare Reach'].unique():
    ...
    for div in Reaches.loc[Reaches['RiverWare Reach']==Reach, 'IDWR Site Code']:
        ...
        try:
            rech = pd.read_csv(f'../Data/Diversions/{Reach}/Recharge/{div}', index_col=0, parse_dates=True)
    ...
    for Station in cols:
        ...
        TrainClimate, TestClimate, TrainDiv, TestDiv = train_test_split(ClimateYear, DiversionsYear, test_size = 0.3, shuffle=False)
